...
public class DriveMode
{
	...
	private static int color_state_prev;											// 直前に取得したカラーセンサ値
	private static int white_scan_count = 0;										// メビウスモード時の旋回速度補正値
	private static int mobius_pass_flag = 0;										// 小さい円通過を示すフラグ
	 ...
	 * 黒検出で左回転、
	 * 白検出で右回転する
	 * @return sstate 　次のループ時に実行する走行モードの値
	 * 
	 */
	public int InLineDrive()
	{
		...
		int color = sensor.getState();								 // 直前のカラーセンサ値
		...
		if( color_state_prev != color )								 // 最新のセンサ値と直前の値が異なっていたら
		{
			...
			case COLOR_ID_WHITE:
				if( mobius_pass_flag == 0 )
					wheel.setSpeed( NORMAL_SPEED_1 , NORMAL_SPEED_2 + 40 );
				else
		...
		if( line_lost_time > 10000 )						 		 // 黒線を見失っている時間が10000カウントを超えたら
	...
	public int SearchLine()
	{
		...
		if( color == COLOR_ID_WHITE )				// 黒線を見つけるまで継続
		{
			wheel.TurnRight();
			Delay.msDelay( 200 );
			wheel.stop();
		...
		else if( color == COLOR_ID_RED )
			wheel.stop();							// 緊急停止
		else if( color == COLOR_ID_BLACK )
		{
			sstate = 0;
